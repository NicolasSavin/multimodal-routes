...
VO_HINT = (
    "Контекст: ведомственная охрана, не ЧОП. Закон — 77-ФЗ от 14.04.1999 «О ведомственной охране». "
    "Работник ВО — гражданин РФ от 18 лет, годный по здоровью и деловым качествам. "
    "Ст. 11: требовать пропускной режим; проверять документы на объекте; досмотр транспорта на объекте; "
    "задерживать нарушителя на охраняемом объекте и передавать в полицию; сила, спецсредства и оружие — только в случаях закона. "
    "Ст. 12: защищать объект/груз; не допускать хищения; при ЧП пресечь, сообщить в полицию и руководству; беречь оружие. "
    "Сопровождение груза: ехать по наряду, удостоверение и документы на груз при себе; ночёвка — гостиницы списка сайта; "
    "груз без охраны не оставлять; ЧП — 112 и диспетчер, затем рапорт. Оружие — только при нападении/угрозе по закону."
)
...
def is_vo_q(q):
    low = (q or "").lower().replace("ё", "е")
    keys = (
        "охран", "77-фз", "77 фз", "ведомствен", "сопровожд", "груз",
        "задержан", "спецсред", "оруж", "рапорт", "чп", "тревог",
        "диспетчер", "наряд", "удостоверен", "объект", "пропуск",
        "караул", "служебн", "пост",
    )
...
def chat():
    ...
    try:
        ...
        if named:
            h0 = named[0]
            more_q = 'гостиница "{name}" {addr} телефон'.format(
                name=h0.get("n") or "",
                addr=h0.get("a") or h0.get("c") or "",
            )
            more, src2 = web_lookup(more_q, skip_wiki=True)
            if more:
                web_rows = more + web_rows
                web_src = src2 + "+" + web_src
            seen, uniq = set(), []
            for w in web_rows:
                k = (w.get("url") or w.get("title") or "")[:90]
                if not k or k in seen:
                    continue
                seen.add(k)
                uniq.append(w)
            web_rows = uniq[:8]
        if is_vo_q(q):
            vo_q = (q + " ведомственная охрана 77-ФЗ").strip()
            more, src2 = web_lookup(vo_q)
            if more:
                seen = set((w.get("url") or w.get("title") or "")[:90] for w in web_rows)
                for w in more:
                    k = (w.get("url") or w.get("title") or "")[:90]
    ...
    sys = (
        "Ты штатный ИИ-помощник «НТЦ Охрана / Команда охрана грузов». "
        "Обсуждай и решай ВСЁ по ведомственной охране: сопровождение и охрана грузов; "
        "командировки; поезд, автобус, купе, нижние места; гостиницы сайта; кто где сегодня; "
        "документы, наряд, удостоверение; права и обязанности по 77-ФЗ; действия на объекте и в пути; "
        "ЧП, тревога, задержание в рамках закона; рапорт диспетчеру; спецсредства и служебное оружие только по закону; "
        "взаимодействие с полицией. Отвечай как старший смены: что сделать и в каком порядке. "
        "Не касса. Запрещено: «нет доступа к билетам»; ссылки tutu, rzd, rasp.yandex.ru, 101hotels, ostrovok, booking. "
        "Не учи преступлениям и не советуй нарушать закон. "
        "Порядок: 1) сначала сайт (рейсы, Гостиницы, кто где) — если в карточке телефон/адрес, бери их. "
        "2) Нет на сайте, пусто или нерелевантно — интернет, не отказывайся искать. "
        "3) Гостиница из списка = наша точка, не тёзка в другом городе (наша «Каспий» ≠ Махачкала). "
    ...
    if named:
        extra.append(
            "Карточка из раздела Гостиницы (если в ней нет телефона/адреса — бери интернет):\n"
    ...
    if web_rows:
        ...
        extra.append("Интернет (если на сайте пусто или не то, используй это):\n" + "\n".join(lines))
    else:
        extra.append("Интернет сейчас ничего не вернул. Если на сайте пусто — так и скажи, не выдумывай.")
    user = q + "\n\n" + "\n".join(extra)
    r = requests.post(
