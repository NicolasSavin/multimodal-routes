...
def hotels_match(q, limit=14):
    ...
    for h in HOTELS:
        ...
        if any(w in blob for w in words) or any(w in low for w in blob.split() if len(w) >= 4):
            hits.append(h)
    if not hits and re.search(r"гостин|отел|ночлег|прожив", low):
        hits = HOTELS[:limit]
    out = []
    for h in hits[:limit]:
...
def web_lookup(q):
    ...
    if len(q) < 3:
        return [], "skip"
    short = re.sub(
        r"\b(как|доехать|добраться|где|какой|какая|какие|есть|нужно|подскажи|пожалуйста|скажи|можно|телефон|адрес|номер|погода|сайт|официальный)\b",
        " ",
        q,
        flags=re.I,
    )
    ...
    rows, srcs = [], []
    plan = (
        (yandex_web_search, q, "yandex"),
        (osm_search, short, "osm"),
        (wiki_search, short, "wiki"),
        (ddg_search, short, "ddg"),
    )
...
def chat():
    ...
    sys = (
        ...
        "Это не касса и не сторонний сервис. Запрещено писать «нет доступа к билетам» и "
        "отправлять на tutu/rzd/яндекс. "
        "Сначала используй данные сайта: живые рейсы, гостиницы, кто где сегодня. "
        "Если ниже есть блок «Интернет» — бери оттуда факты, которых нет на сайте "
        "(адрес, телефон, как доехать, погода, вокзал, гостиница не из списка). "
        "Коротко укажи источник. Не выдумывай адреса и телефоны, если их нет в данных. "
        "Если есть живые рейсы — перескажи номер, тип, время, пересадку. "
        "Если рейсов нет — дай карту ног (пример: Ижевск→Агрыз электричка, Агрыз→Новосибирск поезд, "
        "автобус до Барнаула) и скажи открыть раздел «Маршруты» на этом же сайте, фильтры купе/нижние. "
        "Если есть гостиницы сайта — назови название, адрес, телефон, цену в отчёт и проживание. "
        "Билеты не продаёшь. Отвечай по-русски подробно, 4–8 абзацев."
    )
    ...
    if web_rows:
        ...
        extra.append("Интернет, открытые источники (" + web_src + "):\n" + "\n".join(lines))
    user = q + "\n\n" + "\n".join(extra)
    r = requests.post(
